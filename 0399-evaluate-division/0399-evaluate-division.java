class Solution {
    ...
    private double dfs(String src, String target, double prod, HashSet<String> visited, HashMap<String, HashMap<String, Double>> map){
        double ret = -1;
        visited.add(src);
        if(map.get(src).containsKey(target)){
            ...
            for(String neighbour : map.get(src).keySet()){
                if(!visited.contains(neighbour)){
                    prod *= map.get(src).get(neighbour); 
                    ret = dfs(neighbour, target, prod, visited, map);
                    prod /= map.get(src).get(neighbour); 
