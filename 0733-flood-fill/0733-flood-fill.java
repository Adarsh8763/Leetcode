class Solution {
    ...
    private void dfs(int row, int col, int[][] image, boolean[][] visited, int newColor, int currColor){
        if(row < 0 || row >= rows || col < 0 || col >= cols || visited[row][col] || image[row][col] != currColor){
            return;
        }
        ...
        for(int[] neighbour : adj){
            ...
            dfs(neighbour[0], neighbour[1], image, visited, newColor, currColor);
        }
    }
}